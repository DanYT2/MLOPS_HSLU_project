...
def box(ax, x, y, w, h, label, color, fontsize=9):
    p = FancyBboxPatch(
        ...
        linewidth=1.2, edgecolor=PALETTE["edge"], facecolor=color,
    )
    ax.add_patch(p)
    ax.text(x + w / 2, y + h / 2, label, ha="center", va="center",
            fontsize=fontsize, color=PALETTE["text"], wrap=True)
...
ax.set_title("Customer Churn MLOps — System Architecture",
...
# Training (offline)
box(ax, 0.3, 3.8, 2.0, 1.2, "Training Container\n(train.py)\nLightGBM + XGBoost\n5-fold CV + Optuna",
    PALETTE["compute"], fontsize=8.5)

# MLflow
box(ax, 3.0, 3.8, 2.3, 1.2, "MLflow Server\n(mlflow:5001)\nTracking + Registry",
    PALETTE["store"])

# Postgres
box(ax, 3.0, 1.6, 2.3, 1.0, "PostgreSQL\n(postgres:5432)\nmonitoring_metrics",
    PALETTE["store"])

# API
box(ax, 6.0, 3.8, 2.3, 1.2, "FastAPI Service\n(api:8000)\n/predict, /predict/batch\nChurnEnsembleModel",
    PALETTE["serve"], fontsize=8.5)
...
# Grafana
box(ax, 9.0, 3.8, 2.5, 1.2, "Grafana\n(grafana:3000)\nDrift + Quality Dashboard",
    PALETTE["ops"])
...
arrow(ax, 5.3, 2.4, 6.0, 4.0, rad=0.1, label="batch replay")   # postgres? actually api->monitor
arrow(ax, 7.1, 2.6, 7.1, 3.8, label="/predict/batch", rad=-0.1)
arrow(ax, 8.3, 2.1, 9.0, 2.1, label="SQL")                     # postgres -> adminer? actually we go pg->grafana
...
ax.set_title("Training Pipeline: CSV → Feature Engineering → HPO → CV → Registry",
             fontsize=13, fontweight="bold", pad=10)
...
box(ax, 5.3, 0.4, 1.5, 1.0, "Per-fold metrics,\nconfusion matrix,\nfeat importance",
    PALETTE["store"], fontsize=7.5)
...
box(ax, 0.3, 2.1, 2.2, 0.9, "Weekly cron\n(Mon 05:00 UTC)", PALETTE["data"], fontsize=8.5)
box(ax, 0.3, 0.9, 2.2, 0.9, "Manual dispatch", PALETTE["data"])

# Workflow column
box(ax, 4.0, 4.8, 2.5, 0.7, "ci.yml\nRuff + pytest + cov ≥ 50%", PALETTE["compute"], fontsize=8.5)
box(ax, 4.0, 3.9, 2.5, 0.7, "docker.yml\nBuild + push 3 GHCR images", PALETTE["compute"], fontsize=8.5)
box(ax, 4.0, 3.0, 2.5, 0.7, "release.yml\nSemver tag & release", PALETTE["compute"], fontsize=8.5)
box(ax, 4.0, 2.1, 2.5, 0.7, "secret-scan.yml\nGitleaks SARIF report", PALETTE["compute"], fontsize=8.5)
box(ax, 4.0, 1.2, 2.5, 0.7, "train.yml\nEnd-to-end training run", PALETTE["compute"], fontsize=8.5)
box(ax, 4.0, 0.3, 2.5, 0.7, "CodeQL (default setup)", PALETTE["compute"], fontsize=8.5)

# Outputs column
box(ax, 8.5, 4.5, 3.2, 0.9, "PR check status\n(blocks merge if red)", PALETTE["serve"], fontsize=8.5)
box(ax, 8.5, 3.3, 3.2, 0.9, "GHCR images +\nSBOM + SLSA provenance", PALETTE["store"], fontsize=8.5)
box(ax, 8.5, 2.1, 3.2, 0.9, "Code-scanning alerts\n(SARIF in Security tab)", PALETTE["ops"], fontsize=8.5)
box(ax, 8.5, 0.9, 3.2, 0.9, "submission.csv +\nmlruns artifact", PALETTE["store"], fontsize=8.5)

# Trigger -> workflow arrows
for ty, wy in [(4.95, 5.15), (4.95, 4.25), (3.75, 3.35), (3.75, 2.45),
               (2.55, 3.35), (2.55, 0.65), (1.35, 1.55)]:
